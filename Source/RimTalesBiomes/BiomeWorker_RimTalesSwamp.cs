...
using Verse;
public class BiomeWorker_RimTalesSwamp : BiomeWorker
{
    public override float GetScore(BiomeDef biome, Tile tile, PlanetTile planetTile)
    {
        ...
        if (Rand.ValueSeeded(planetTile.tileId ^ 0x11245d7a) > 1f)
        {
            return 0f;
        }
        // A formula to calculate the score at this tile.
        return (float)(29f + (tile.temperature - 20f) * 1.5f + (tile.rainfall - 300f) + (tile.swampiness - 300f) / 120f);
    }
}